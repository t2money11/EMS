...
<sf:form id="form1" method="post" action="/EMS/inquiry/update" modelAttribute="inquiry">
...
<div class="container" id="pagetop">
	...
	<div class="titledbox">
		...
		<div class="titledbox-body titledbox-body-extended">
			...
			<div class="form-actions ta-c">
				<a class="btn btn-primary w150" id="updateSave">
					<b>
						保存询单
					</b>
				</a>
							...
							<c:forEach var="item" items="${inquiry.inquiryDtlList}" varStatus="status">
								...
								<tr>
									<td class="va-m ta-c" rowspan="6">
										<c:choose>
										<c:when test="${inquiry.inquiryDtlList.get(status.index).isUsed}">
										</c:when>
										<c:otherwise>
								...
								<tr style="height: 55px">
									...
									<td>
										<c:choose>
										<c:when test="${inquiry.inquiryDtlList.get(status.index).isUsed}">
											<sf:input class="control-largeCommon" path="inquiryDtlList[${status.index}].finalPrice" maxlength="11" style="width:120px" readonly="true"/>
										</c:when>
										<c:otherwise>
