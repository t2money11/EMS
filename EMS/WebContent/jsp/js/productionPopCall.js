...
//订单用
$(function () {
    $("#productionRef").click(function (e) {
    	...
    	var url = "/EMS/production/searchPopShow?customerId=" + customerId + "&customerName=" + customerName;
    	url=encodeURI(url);
        var whObj = { width: 1000, height: 850 };
        var returnData = openShowModalDialogP(url, null, whObj, e);
        if (returnData) {
        	$("#form1").attr("action", "/EMS/" + this.getAttribute("funcId") + "/addProduction");
...
//询单用
$(function () {
    $("#productionRef4I").click(function (e) {
        ...
        var returnData = openShowModalDialogP(url, null, whObj, e);
        if (returnData) {
        	$("#form1").attr("action", "/EMS/inquiry/addProduction");
    		$("#form1").attr("method", "post");
    		document.getElementById("productions").value = returnData;
    		$(window).unbind('beforeunload');
    		$("#form1").submit();
