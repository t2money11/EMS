...
//发货单用
$(function () {
    $("#traderOrderRef").click(function (e) {
    	var customerId = $("input[name='customerId']")[0].value;
    	var customerName = $("input[name='customerName']")[0].value;
    	if(customerId == ""){
    		alert("添加订单产品之前，请先选择客户");
    		return;
    	}
    	var url = "/EMS/tradeOrder/searchPopShow?customerId=" + customerId + "&customerName=" + customerName;
    	url=encodeURI(url);
        var whObj = { width: 1000, height: 850 };
        var returnData = openShowModalDialogP(url, null, whObj, e);
        if (returnData) {
        	$("#form1").attr("action", "/EMS/receipt/addProduction");
